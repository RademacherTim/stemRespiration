...
```{r installDependencies, echo = T}
install.packages ('segmented', repos = 'http://cran.us.r-project.org')
install.packages ('tibble',    repos = 'http://cran.us.r-project.org')
```
``` {r loadDependencies, echo = T}
library ('segmented')
library ('tibble')
fileList <- list.files (path = normalizePath ('../RespChamberProc/R/'))
# Nota bene: Brooklynn, you do not need to drop the 5th element. The commented line below should work fine. 
fileList <- paste ('./', fileList [-5], sep = '')
#fileList <- paste ('./', fileList, sep = '')
res <- sapply (fileList, source, .GlobalEnv)
```
...
dat$ea.Pa   <- rep (ea.Pa,   length (dat [, 1]))   # add actual water vapour pressure [Pa] to raw data.frame
dat$airt.C  <- rep (airt.C,  length (dat [, 1]))   # add air temperature [degC] to raw data.frame
dat$pres.Pa <- rep (pres.Pa, length (dat [, 1]))   # add atmospheric pressure [Pa] to raw data.frame
dat$H2O.ppt <- dat$ea.Pa / (dat$pres.Pa - dat$ea.Pa) * 1.0e3   # add actual water vapour pressure [ppt] to raw data.frame
```

## Process data to estimate respiration flux 
``` {r addTIMESTAMP, include=FALSE}
# Add a column with time in seconds
dat$TIMESTAMP <- dateAndTime + dat$Time.S 
```
...
From the dry concentration of CO<sub>2</sub> in dry air (` 'CO2.dry'`), the chamber size, the air temperature  (` 'tair.C'`) and the atmospheric (`'pres.Pa'`) at the time we can calculate the respiration flux using the  `'calcClosedChamberFlux'` of the `'RespChamberProc'` package.
...
We want to plot the resulting fit, to look at it again. 
``` {r plotFit, echo = T}
plotResp (dat, 
          resFlux = resFit,
          colConc = 'CO2.dry',   # redundant
          colTime = 'TIMESTAMP', # redundant
          label = paste (studyName, paste (treeID,'p',treatment,'p',chamberNumber,sep = ''),dateAndTime))
```

If there is no problem, we want to save the plot and the fit to a data structure with the results in the `'fig/'` directory. 

``` {r savePlotFit, echo = T}
fileName2 <- substr (fileName, 1, nchar (fileName)-4)
png (file = paste ('fig/',fileName2,'.png', sep = ''))
plotResp (dat, 
          resFlux = resFit,
          colConc = 'CO2.dry',   # redundant
          colTime = 'TIMESTAMP', # redundant
...
Now, we can add the resulting fit (`'resFit'`) to the data.frame containning site names, etc.

## Processing multiple files

If you have multiple files, you may want to analyse all of them and add the results to a single data.frame. For the example, here I assume that all `csv` files in the `data/` directory are intended to be analysed. First, you make a list of all files in the `data/` directory.

``` {r readInFileNames}
# Create list of all csv files in the data/ directory 
fileNames <- list.files (path = '../data/', pattern = '.csv')
```

Next, dates and times for all files are extracted.

``` {r getDatesAndTimes}
# Isolate the important part of the string
...
# put the dates and times together
datesAndTimes <- strptime (paste (datestr, timestr), "%Y%m%d %H%M%S")
```

And finally the data.frame for all results is created.

``` {r createDataFrame}
# Create the data.frame
alldata <- data.frame (file = fileNames,date = datesAndTimes, stringsAsFactors = FALSE)
```
