...
#  raw$ea.Pa   <- rep (ea.Pa,   length (raw [, 1]))
#  raw$airt.C  <- rep (airt.C,  length (raw [, 1]))
#  raw$pres.Pa <- rep (pres.Pa, length (raw [, 1]))
#  raw$H2O.ppt <- raw$ea.Pa / (raw$pres.Pa - raw$ea.Pa) * 1.0e3
```
...
```
We can then correct for the atmospheric water vapour pressure  (`r 'H2O.ppt'`) to get the CO<sub>2</sub> concentration of dry air  (`r 'CO2.dry'`).
...
From the dry concentration of CO<sub>2</sub> in dry air (`r 'raw$CO2.dry'`), the chamber size, the air temperature  (`r 'raw$tair.C'`) and the atmospheric (`r 'raw$pres.Pa'`) at the time we can calculate the respiration flux using the  `r 'calcClosedChamberFlux'` of the `r 'RespChamberProc'` package.
